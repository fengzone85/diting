...
// 单节点按需分析：入口按钮 + 弹窗（弹窗内自带窗口切换、登录态与错误码分支）
      ...
      <div class="space-y-6">
        ...
        <div class="glass p-6">
          <div class="flex items-center justify-between">
            ...
            <button
              class="rounded-lg bg-sky-600 px-3 py-1 text-sm font-medium text-white hover:bg-sky-500"
              @click="showNodeDialog = true"
            >{{ t('ai.analyzeNode') }}</button>
          </div>
          <p class="mt-2 text-xs text-slate-500">{{ t('ai.analyzeNodeHint') }}</p>
        </div>
