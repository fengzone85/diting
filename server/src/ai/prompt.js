...
【负载判断（cpu.cores）】
- load.avg1 是「1 分钟平均负载」，必须结合 cpu.cores 才有意义（1 核机的 0.18 与 8 核机的 0.18 完全不同）。
- 有 load.avg1_per_core 时以其为准；cpu.cores 为 null 或 load.cores_unknown=true 时，**禁止**对该节点负载高低下结论（只能说明"缺少核数、无法判断"）。
