...
- 磁盘「多久写满」基于 `AI_TREND_DAYS`（默认 7 天，范围 2–14）趋势估计，日报中给出区间与置信度；设为 `0` 可回退旧的「24 小时首末两点」算法
- 风险等级有**本地规则锚点**（离线占比/超阈值节点数/临期数），模型只能在锚点上下浮动一级；长期离线节点有「沉默期」（默认 3 天）不再逐台打扰
- 后台「Ai」页可一键手动触发（**异步任务**：接口立即返回，页面轮询运行状态；触发有冷却，避免重复点击重复计费）
- 纯服务端任务，不引入任何下行指令
...
3. **数据最小化** — 只采基础状态，不采指纹信息（内核/CPU型号/GPU/公网IP）
...
**③·补 指纹的风险有两个维度：定位，以及聚类**

常见的直觉是：「反正 Agent 零入站、不采公网IP、不接受指令，采什么都连不上，所以指纹无害。」这个推理只覆盖了第一个维度，漏掉了第二个：

| 维度 | 攻击者要解决的问题 | 靠什么防 | diting 对策 |
| --- | --- | --- | --- |
| **定位** | 这台机器在哪？怎么连上？ | 不采公网IP / IP 地址 | ✅ 不采 IP，且零入站 |
| **聚类** | 我已落地一台，**哪些机器可以用同一套 exploit 再打一遍？** | 不采可匹配漏洞的指纹 | ✅ 不采内核版本、CPU 型号、GPU |

关键区别：**聚类不需要定位能力**。攻击者如果已经通过完全无关的入口（某台机器上一个老 CMS）拿到了一台的 shell，他会想知道「我手上这套 exploit 还能打你的哪几台」。此时 DB 里任何能把机器分组的字段都会降低他的二次扩散成本：

- **内核版本** → 直接匹配 CVE（`3.10.0-1160` ⇒ 该打的漏洞清单）；
- **CPU 型号** → 识别微架构（`E5-2680 v4` ⇒ Skylake-SP ⇒ MDS/Downfall 类；`EPYC 7B13` ⇒ Zen 系列），同型号机器漏洞面完全相同，**攻陷一台 = 拿到该型号所有机器的攻击配方**；
- **监听端口 / 进程列表** → 直接就是一份「哪台跑着 openssh 7.4 / nginx 1.14」的待办清单。

所以「无公网IP」防的是「找不到你」，「不采内核/CPU型号」防的是「找到之后知道打哪里」——这是两条独立的防线，前者不能替代后者。本项目两者都做，且这条纪律**不因单点可否被直接利用而开口子**：一旦为「反正连不上」开了第一个例外，这条线就守不住了。

> 补充说明：`hostname` 与 `os`（如 "Ubuntu 22.04"）确实已被存储，属于轻量标识。前者是运维刚需（区分节点），后者粒度粗（发行版名不能直接匹配内核 CVE）。我们明确划的线是：**不采集能直接匹配漏洞或降低二次扩散成本的字段**，而非「不采集任何字符串」。
...
3. **关于「聚类」维度**：见上方 ③·补。「无公网IP」只解决「定位」，不解决「攻击者已落地一台后，用 DB 里的指纹筛出其它同型号机器再打一遍」。因此我们不采内核版本/CPU 型号/GPU/进程数/连接数——不是因为这些字段在「无公网IP + 无指令通道」下能被直接利用（它们确实不能），而是因为它们会降低攻击者的二次扩散成本。这条纪律同样解释了为什么**新增采集字段需要重新做安全评审**，而非按「反正连不上」逐项开例外。
...
| 聚类风险 | ❌ 无（不采内核/CPU型号，攻击者无法按微架构分组） | ⚠️ 有（内核/型号可将机群分桶，复用同一 exploit） |
