...
> **⚠️ Upgrade note (Breaking since 2026-09-07)**: the one-line install-command endpoints (create agent / reset token / generate commands) **no longer derive the server address from the `Host` / `X-Forwarded-Host` header**. Configure exactly one of: ① Dashboard "Settings → Agent server URL" (recommended); ② Dashboard "Site URL"; ③ the `PUBLIC_URL` env var — otherwise they return `400 server_url_not_configured`. Agent reporting is unaffected.

**Server `.env`**: `PORT`, `ADMIN_TOKEN`, `SETUP_TOKEN` (optional self-registration), `SESSION_SECRET`/`SESSION_TTL_MS` (2FA session — see "Two-factor authentication (TOTP)"), `READONLY_TOKEN` (optional, read-only account — see below), `ADMIN_ALLOW_HTTP` (plaintext opt-in for intranet only), `PUBLIC_URL` (server public URL used to generate agent install commands; can also be set in the dashboard "Settings → Agent server URL" — see the upgrade note above), `AGENT_INTERVAL` (expected report interval), `OFFLINE_THRESHOLD_SEC` (default 60), `RETENTION_DAYS` (default 30), `PROBES_DOWNSAMPLE`/`PROBES_MAX_POINTS` (latency-waveform backend downsampling, default 1 / 5000), `ALERT_CPU_PCT`/`ALERT_MEM_PCT` (default 90), `ALERT_COOLDOWN_SEC`, `SMTP_*` (QQ Mail alerts), `TELEGRAM_BOT_TOKEN`/`TELEGRAM_CHAT_ID` (optional, Telegram alerts).

**`TRUST_PROXY`** (new since 2026-09-10, default `loopback`): which reverse proxy the server trusts when deriving `req.ip` (used for the admin IP whitelist, per-IP rate limits and audit logs). Keep the default when Nginx runs on the same host and 8081 binds `127.0.0.1`. If the proxy lives on another host or in another container, set it explicitly — e.g. `TRUST_PROXY=172.16.0.0/12,10.0.0.8` (comma-separated IPs/CIDRs) or `TRUST_PROXY=1` (trust one hop). **Upgrade note**: the previous value was `trust proxy = true` (trust any client-supplied `X-Forwarded-*`), which let anyone reaching `:8081` directly spoof `X-Forwarded-For` to bypass the IP whitelist, or spoof `X-Forwarded-Proto: https` to use admin endpoints over plaintext HTTP. After upgrading, deployments whose proxy is not on loopback **must** set `TRUST_PROXY`, otherwise `req.ip` becomes the proxy IP and a configured whitelist will reject every request.

**Public data boundary (`public_show_business`)**: `/api/public/agents` and `/api/v1/nodes` expose business fields (provider, expiry, note, monthly quota, plan) by default, because the public home page ("providers / expiring soon") and the node detail page ("note / plan") render them. Turn the switch off in Dashboard → Settings → "Expose business fields on public API" when your public page faces outside visitors; the fields then disappear from the response entirely. This is independent of "Enable public page" (`public_enabled`): when the public page is disabled, both API families return 403.

**Monitored side**: `SERVER_URL`, `AGENT_ID`, `AGENT_TOKEN`, `INTERVAL` (seconds, default 15 for the Python agent, 20 for the Go binary), `DISK_PATH` (default `/`), `PROBE_TARGETS` (network-quality self-test targets; default `CM:www.10086.cn:443,CT:101.226.4.6,CU:202.106.0.20,GG:8.8.8.8` — carrier DNS servers reject TCP outright, so `CM` probes the China Mobile web portal; empty to disable; format `label:host[:port]`, comma-separated). The Go binary adds: `STATE_FILE` (monthly-traffic state; default `/data/state.json`), `ADAPTIVE` (default on; auto switches `FAST_INTERVAL`=10s ↔ `SLOW_INTERVAL`=60s by local change-rate), `GZIP` (off by default; needs server-side decompression middleware or reports are rejected with 400), `DEBUG` (verbose logs, never prints the token). With `ADAPTIVE` on, set the server `OFFLINE_THRESHOLD_SEC=120` (or above the slow interval) so steady-state agents are not flagged offline.
