...
- 多个目标用逗号分隔，格式 `label:host[:port]`（如 `CM:211.136.192.6`）
- ICMP 优先，失败自动 TCP 回退（默认探测 53 端口）
- **本地固定，服务端不可下发** — 这是核心安全设计
- `label` 建议用 ≤2 字符缩写（`CM`=移动 / `CT`=电信 / `CU`=联通 / `GG`=公共谷歌 DNS），状态卡片空间有限；默认 `PROBE_TARGETS=CM:211.136.192.6,CT:101.226.4.6,CU:202.106.0.20,GG:8.8.8.8`

### 网络质量探测示例

```bash
# 探测三家运营商 DNS + 公共 DNS（默认，label 用两字母缩写）
PROBE_TARGETS=CM:211.136.192.6,CT:101.226.4.6,CU:202.106.0.20,GG:8.8.8.8
